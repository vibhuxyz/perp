...
* **Maintenance Margin Ratio (MMR) without floats:** A percentage in `bigint` land is stored as an integer numerator plus a scale denominator (`2n / 100n` = 2%). You multiply *before* you divide (`notional * RATIO / SCALE`), never the other way round, or truncation eats the answer.
* **Position Equity:** `equity = locked margin + unrealised PnL`. It is the money actually backing the position right now. Liquidation is simply `equity <= maintenanceMargin`. At `equity = 0` the position is bankrupt and the exchange is eating the loss.
* **Mark Price vs Index Price vs Last Traded Price:** Last traded price is our own book (manipulable). Index price is an average of external venues. Mark price is usually a *blend* of index price and our own book's fair basis — it is the number used for PnL and liquidation, so it must be hard to push around.
* **Funding Rate:** A perpetual has no expiry, so nothing forces its price toward spot. Funding does it: on a schedule, if the perp trades above the index, longs pay shorts; below, shorts pay longs. It is the economic spring that ties a perp to the underlying.
* **Write-Ahead Log (WAL):** Append the command to durable storage *before* mutating in-memory state. The log is the truth; memory is a cache of it. On restart you replay the log and get the same state back — but only if the engine is deterministic (same inputs -> same outputs), which is why replay and tests are the same problem.
* **Idempotency:** In a distributed system every message can arrive twice (retries, consumer restarts, at-least-once delivery). "Credit this balance" is not safe to run twice. Attaching a unique event id and recording which ids you've already applied turns an at-least-once pipeline into effectively-once.
* **Reconciliation:** Never trust that two ledgers agree just because the code says so. A background job that re-derives total user equity from the position ledger and compares it with the wallet ledger catches slow leaks (truncated remainders, missed unlocks) that no single unit test would.
* **Insurance Fund & ADL:** If price gaps past the liquidation price, the forced close fills worse than the bankruptcy price and the loss is real. The insurance fund (built from liquidation penalties) absorbs it. When the fund is drained, exchanges fall back to Auto-Deleveraging: forcibly closing the most profitable, highest-leverage traders on the other side. Without either, one violent tick makes the exchange insolvent.
* **Where Liquidation Bugs Hide:** In the race between "price update" and "margin check". A position can be checked as safe, then the price moves, then an order is accepted against stale margin. Load tests that hammer rapid price updates *concurrently* with order flow are what surface this — a single-threaded happy-path test never will.
