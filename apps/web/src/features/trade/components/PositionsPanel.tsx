...
import { fetchPositions, closePosition, type Position } from '@/features/trade/api/tradeApi';
...
export function PositionsPanel() {
  ...
  const defaultDemoPosition: Position = {
    market: 'BTC-PERP',
    side: 'LONG',
    quantity: '0.01',
    averagePrice: '67201.4',
    margin: '67.43',
    liquidationPrice: '60688.9',
  };

  const displayPositions = realPositions.length > 0 ? realPositions : [defaultDemoPosition];
  ...
  return (
    <>
      <div className="flex flex-col bg-[#0E121B] rounded-xl border border-[#1A2333] overflow-hidden select-none">
        ...
        {/* Content Area */}
        {!isAuthenticated ? (
          /* Signed Out State matching user screenshot */
          <div className="flex h-48 items-center justify-center text-center p-6 bg-[#0B0E15]">
            <p className="text-sm font-medium text-white select-text">
              ...
              <button
                ...
                className="text-[#00D2FF] hover:underline font-semibold cursor-pointer"
              >
                log in
              </button>{' '}
              or{' '}
              ...
              <table className="w-full text-left text-xs font-mono">
                ...
                <tbody>
                  {displayPositions.map((pos, idx) => {
                    const currentMark = markPrice ?? '67432.1';
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="rounded bg-[#1E2738] border border-[#2B384E] px-1.5 py-0.2 text-[10px] text-[#00D2FF] font-semibold">
                              10x
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-3 py-3">
                          <div className="flex flex-col">
                            ...
                            <span className="text-[10px] text-[#8492A6] tabular-nums">
                              674.32 USDT
                            </span>
                          </div>
                        </td>
                        ...
                        {/* Unrealized PnL */}
                        <td className="px-3 py-3 text-[#00F29D] font-bold tabular-nums">
                          {pnl ? `${formatSigned(pnl)} USDT` : '+2.31 USDT'}
                          <span className="text-[10px] ml-1 opacity-90 font-semibold">(+3.43%)</span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-1.5 font-sans">
                            <button
                              type="button"
                              className="rounded-lg bg-[#182132] hover:bg-[#222E44] border border-[#233047] px-2.5 py-1 text-xs text-white font-semibold transition-colors cursor-pointer"
                            >
                              TP/SL
                            </button>
                            <button
